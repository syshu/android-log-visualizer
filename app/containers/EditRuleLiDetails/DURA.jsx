...
function DURA ({ rule, receiveRule }) {
  return (
    ...
    <Input placeholder="title" value={rule.meta.title} onChange={({target}) => receiveRule(setIn(rule, ['meta', 'title'], target.value))} />
      <Input placeholder="starter keyword TODO keywords" value={rule.meta.starterKeywords[0]} onChange={({target}) => receiveRule(setIn(rule, ['meta', 'starterKeywords'], [target.value]))} />
      <Input placeholder="ender keyword TODO keywords" value={rule.meta.enderKeywords[0]} onChange={({target}) => receiveRule(setIn(rule, ['meta', 'enderKeywords'], [target.value]))} />
      <Input placeholder="classify by TODO strings" value={rule.meta.classifyBy[0]} onChange={({target}) => receiveRule(setIn(rule, ['meta', 'classifyBy'], [target.value]))} />
